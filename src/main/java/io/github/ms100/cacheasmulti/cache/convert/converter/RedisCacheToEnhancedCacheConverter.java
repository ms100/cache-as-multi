...
@Component
public class RedisCacheToEnhancedCacheConverter implements EnhancedCacheConverter<RedisCache> {

    @Override
    public EnhancedCache convert(RedisCache source) {
        return new EnhancedRedisCache(source.getName(), source.getNativeCache(), source.getCacheConfiguration());
    }

    static class EnhancedRedisCache extends RedisCache implements EnhancedCache {

        private static final String CACHE_WRITER_CLASS = "org.springframework.data.redis.cache.DefaultRedisCacheWriter";
        private final Method executeMethod;

        @SneakyThrows
        protected EnhancedRedisCache(String name, RedisCacheWriter cacheWriter, RedisCacheConfiguration cacheConfig) {
